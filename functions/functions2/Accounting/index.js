...
function addExpense(expTo) {
  return docExpense
    ...
    .then(() => {
      console.log('doc month Exp', docTotalExpense)
...
function addExpenseMonth(expTo) {
  return docExpenseMonth
    ...
    .then(() => {
      console.log('doc month', docMonthExpense)
...
function setData() {
  if (snapData.entry_type == 'Asset Allocation') {
    ...
    assTo.ass = {
      val: Number(snapData.total_price),
      data: [Number(snapData.total_price)]
    }
    ...
    expTo.exp = {
      val: Number(snapData.total_CTC),
      data: [Number(snapData.total_CTC)]
    }
    ...
    eqTo.eq = {
      val: Number(snapData.total_profit),
      data: [Number(snapData.total_profit)]
    }
    ...
    dataToUse = { ...incTo, ...expTo, ...eqTo, ...assTo }
    addBook()
    addAssets(assTo)
    addExpense(expTo)
    ...
    let sub_type = ''
    let sub_sub_type = ''
    let ass_name = snapData.cart_items.product_name
    let ass = snapData

    assTo.ass = {
      val: 0,
      data: [Number(snapData.total_price), -Number(snapData.total_price)]
    }
    ...
    assTo[ass_name] = {
      val: Number(snapData.total_price),
      data: [Number(snapData.total_price)],
      qty: Number(snapData.total_quantity),
      price: Number(snapData.total_price)
    }
