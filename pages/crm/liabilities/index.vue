    ...
    <vx-card class="mt-12">
      ...
      <div class="flex flex-wrap items-center justify-between">
        ...
        >
          ...
          >
          <addingAssets />
          <vs-button @click="$router.push('/crm/liabilities/breakdown')"
            >BreakDown</vs-button
          >
        </div>
        <vs-row class="mt-6">
...
export default {
  ...
  computed: {
    ...
    liab_indv() {
      return this.$store.state.crm.liab_indv
    },
    company_assets() {
      let ass = []
  ...
  mounted() {
    ...
    this.$fireStore
      ...
      .then(function(doc) {
        if (doc.exists) {
          let assets = doc.data()
          vm.$store.commit('crm/LIAB_UPDATE', assets)
          console.log('eya')
        } else {
    ...
    docR.onSnapshot(snapshot => {
      snapshot.docChanges().forEach(change => {
        ...
        assets.push({
          ...
          uid: doc.data().uid,
          liab_name: doc.data().f_liab,
          liab_type: doc.data().liab_type,
          documentation: doc.data().docs,
          branch: doc.data().branch,
          price: doc.data().t_liab,
          quantity: doc.data().qty,
          reference: doc.data().ref
        })
      })
      vm.$store.commit('crm/LIAB_INDV_UPDATE', assets)
    })
  }
}
</script>
