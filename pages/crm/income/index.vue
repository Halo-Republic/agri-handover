    ...
    <vx-card class="mt-12">
      ...
      <div class="flex flex-wrap items-center justify-between">
        ...
        >
          ...
          >
          <!-- <addingAssets />
          <vs-button @click="$router.push('/crm/expenses/breakdown')"
            >BreakDown</vs-button
          > -->
        </div>
        <vs-row class="mt-6">
...
import CellRendererAction from '@/components/cell-renderer/action_invoice.vue'
...
export default {
  ...
  computed: {
    total_exp() {
      return this.$store.state.crm.total_exp
    },
    inc_indv() {
      return this.$store.state.crm.inc_indv
    },
    company_assets() {
      let ass = []
  ...
  mounted() {
    ...
    //     if (doc.exists) {
    //       let assets = doc.data()
    //       vm.$store.commit('crm/EXP_UPDATE', assets)
    //       console.log('eya')
    //     } else {
    ...
    docR = this.$fireStore
      ...
      .doc('sales')
      .collection('sales')
    // .where('entry_type', '==', 'Profit Allocation')
    ...
    docR.onSnapshot(snapshot => {
      snapshot.docChanges().forEach(change => {
        ...
        assets.push({
          ...
          uid: doc.data().uid,
          c_displayname: doc.data().c_displayname,
          product: doc.data().addItems,
          price: doc.data().price,
          t_ctc: doc.data().t_ctc,
          t_price: doc.data().t_price,
          t_comm: doc.data().t_comm,
          t_profit: doc.data().t_profit,
          t_quantity: doc.data().t_quantity,
          m_payment_id: doc.data().m_payment_id,
          date: doc.data().date
        })
      })
      // .then(() => {
      console.log('assets', assets)
      this.$store.commit('crm/INC_INDV_UPDATE', assets)
      // })
    })
  }
}
