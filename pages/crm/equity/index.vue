    ...
    <vx-card class="mt-12">
      ...
      <div class="flex flex-wrap items-center justify-between">
        ...
        >
          ...
          >
          <addingAssets />
          <vs-button @click="$router.push('/crm/equity/breakdown')"
            >BreakDown</vs-button
          >
        </div>
        <vs-row class="mt-6">
...
export default {
  ...
  computed: {
    total_eq() {
      return this.$store.state.crm.total_eq
    },
    eq_indv() {
      return this.$store.state.crm.eq_indv
    },
    company_assets() {
      let ass = []
  ...
  mounted() {
    ...
    this.$fireStore
      ...
      .then(function(doc) {
        if (doc.exists) {
          let assets = doc.data()
          vm.$store.commit('crm/EQ_UPDATE', assets)
          console.log('eya')
        } else {
    ...
    docR = this.$fireStore
      ...
      .doc('book')
      .collection('book')
      .where('entry_type', '==', 'Equity Allocation')
    ...
    docR.onSnapshot(snapshot => {
      snapshot.docChanges().forEach(change => {
        ...
        assets.push({
          ...
          uid: doc.data().uid,
          eq_name: doc.data().eq_type,
          eq_type: doc.data().eq_type,
          documentation: doc.data().docs,
          branch: doc.data().branch,
          price: doc.data().t_cost,
          quantity: doc.data().shares_amount,
          reference: doc.data().ref
        })
      })
      vm.$store.commit('crm/EQ_INDV_UPDATE', assets)
    })
  }
}
</script>
