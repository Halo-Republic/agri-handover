...
import CellRendererAction from '@/components/cell-renderer/action_inv.vue'
...
export default {
  ...
  computed: {
    ...
    exp_indv() {
      return this.$store.state.crm.exp_indv
    },
    company_assets() {
      let ass = []
  ...
  mounted() {
    ...
    // .where('entry_type', '==', 'Expense Allocation')
    ...
    docR.onSnapshot(snapshot => {
      snapshot.docChanges().forEach(change => {
        ...
        assets.push({
          ...
          uid: doc.data().uid,
          t_cost: doc.data().t_cost,
          date: doc.data().created_date,
          items: doc.data().c_items,
          customer: doc.data().c_disp
        })
      })
      vm.$store.commit('crm/EXP_INDV_UPDATE', assets)
    })
  }
}
</script>
