<template>
  <div>
      ...
      <vs-popup title="Pricing Structure" :active.sync="popupActivo">
        <vs-row>
          ...
          >
            <vs-card>
              <div slot="header">
                <h3 class="text-style">Monthly</h3>
              </div>
              <div>
    ...
    <div class="p-6">
      ...
      <h6 class="mt-4">Registration Type</h6>
      <v-select
        class="w-full  mb-5"
        label="Business Type"
        v-model="b_type"
...
export default {
  ...
  methods: {
    ...
    newBusiness() {
      if (process.client) {
        let vm = this
        console.log('wm', vm.google_marker)

        if (vm.radios1 == 'auto') {
          const geohash1 = Geohash.encode(
            `${vm.google_marker.lat}`,
            `${vm.google_marker.lng}`,
            1
          )
          vm.geo1 = geohash1
          ...
          let uniqueName = vm.b_name.toLowerCase().replace(/\s/g, '_')

          let newLogo = ''
          if (vm.uploaded_images[0]) {
            newLogo = vm.uploaded_images[0]
          }
          ...
          if (vm.b_type == 'Individual') {
            businessDetails.b_name = vm.contactperson
            businessDetails.un_name = vm.contactperson
              .toLowerCase()
              .replace(/\s/g, '_')
            businessDetails.b_type = vm.b_type
            businessDetails.id_number = ''
            businessDetails.gender = ''
          } else {
            businessDetails.b_type = vm.b_type
            businessDetails.reg_number = vm.reg_number
            businessDetails.vat_number = vm.vat_number
          }
          ...
          console.log('google man', this.google_man)

          console.log('here google', this.google_man.geometry)
          const geohash1 = Geohash.encode(
          ...
          if (vm.b_type == 'Individual') {
            ...
            businessDetails.b_type = vm.b_type
            businessDetails.b_name = vm.b_name
            businessDetails.un_name = vm.b_name
              .toLowerCase()
              .replace(/\s/g, '_')
            businessDetails.reg_number = vm.reg_number
            businessDetails.vat_number = vm.vat_number
