            ...
            <vx-card class="grid-view-item mb-base overflow-hidden" v-on="$listeners">
                <template slot="no-body">
                    ...
                    <div class="item-img-container bg-white h-64 flex items-center justify-center mb-4 cursor-pointer" @click="navigate_to_detail_view">
                        <img :src="item.images[0]" :alt="item.product_name" class="grid-view-img px-4">
                    </div>
                    <div class="item-details px-4">
                        ...
                        <!-- TITLE & DESCRIPTION -->
                        <div class="my-4">
                            <h6 class="truncate font-semibold mb-1 hover:text-primary cursor-pointer" @click="navigate_to_detail_view">{{ item.product_name }}</h6>
                            <p class="item-description truncate text-sm" v-html="item.description">{{ item.description | str_limit(8)}}</p>
                        </div>
                    </div>
