...
<img :src="img" style="width: 40%" />
    ...
    <!-- SINGLE SELECTION -->
    <vs-button @click="popupActivo4 = true" icon="done" color="primary">{{
      label
    }}</vs-button>
    <vs-popup
      fullscreen
  ...
  methods: {
    ...
    AddCategory() {
      let addingInformationLocation = this.$fireStore
        ...
        .collection('images')
        .doc('category')
        .collection(this.app_active_user.uid)
    ...
    setImages() {
      ...
      for (let i = 0; i < this.images.length; i++) {
        const mountainsRef = storageRef.child(
          `${this.app_active_user.uid}/${this.images[i].name}`
        )
